...
export type KeyConstraint<
	KeySource extends Object,
	T extends Constraint = Constraint
> = [Extract<keyof KeySource, string>, ...T[]];
class Restraint {
	...
	assign(constraint: Constraint): void {
		...
		if (typeof constraint == "boolean") this.nullable = constraint;
		else if (constraint instanceof RegExp) this.pattern = constraint;
		else if (constraint instanceof Function) this.type = constraint;
		else this.length = constraint;
	}
}
...
export function satisfyConstraints<T extends Object>(
	target: T,
	...constraints: KeyConstraint<T>[]
): true | [string, FailureReason] {
			...
			if (target[key].constructor != restraint.type)
				...
				if (typeof target[key] == "string") {
					if (target[key].length < restraint.length[0])
						...
						restraint.length[1] &&
						target[key].length > restraint.length[1]
					)
...
export function validateParameter<ReqQuery>(
	request: Request<ParamsDictionary, any, any, ReqQuery>,
	response: Response,
	...constraints: KeyConstraint<ReqQuery, NonTypeConstraint | typeof Array>[]
): ReturnType<typeof satisfyConstraints> {
	const result = satisfyConstraints(request.query, ...constraints);
	if (result !== true)
		response.status(400).send(`${result[0]} : ${result[1]}`);
	return result;
}
export function validatePayload<ReqBody extends Record<string, any>>(
	request: Request<ParamsDictionary, any, ReqBody>,
	response: Response,
	...constraints: KeyConstraint<ReqBody>[]
): ReturnType<typeof satisfyConstraints> {
